...
def add_chosen_skill(update, user_data):
    ...
    if res == 1:
        ...
        text += "Этот навык еще не готов"    #TODO окончание
...
def set_target(bot, update, user_data):
    ...
    bot.sync_send_message(chat_id= update.message.chat_id, text="Вы выбрали цель, ждем других игроков",
                     reply_markup=cancel_button)        #TODO Сообщение должно быть до следующего сообщение о просчете битвы
    battle.skills_queue.append(player_choosing)
    if battle.is_ready():
        battle_count(battle) #TODO исправить
...
def battle_skip_turn(bot, update, user_data):
    ...
    user_data.update({'status': 'Battle waiting'})
    dispatcher.bot.sync_send_message(chat_id=player_choosing.participant.id, text="Ждем других игроков",
                                reply_markup=cancel_button)     #TODO Сообщение должно быть до следующего сообщение о просчете битвы
    battle.skills_queue.append(player_choosing)
    if battle.is_ready():
        battle_count(battle)  # TODO исправить
...
def battle_count(battle):     #Тут считается битва в которой все выбрали действие, отдельный процесс, Не забыть сделать так, чтобы выполнялось в таком порядке, в котором было выбрано
    ...
    for i in range(2):
        for j in range(battle.team_players_count):
            ...
            player_choosing.target = None
            player_choosing.skill = None
            dispatcher.bot.send_message(chat_id=player.id,
    ...
    for i in range(2):
        for j in range(battle.team_players_count):
            ...
            dispatcher.bot.send_message(chat_id=player.id, text =result_strings[0] + result_strings[1] +
    ...
    if res != -1:
        for i in range(2):
            for j in range(battle.team_players_count):
                player_choosing = battle.teams[i][j]
                player = player_choosing.participant
                interprocess_dictionary = InterprocessDictionary(player.id, "user_data", {'status': dispatcher.user_data.get(player.id).get('saved_battle_status')})
                interprocess_queue.put(interprocess_dictionary)
                dispatcher.bot.send_message(chat_id=player.id, text="{0} команда победила!".format("Первая" if res == 0 else "Вторая"))
                show_general_buttons(dispatcher.bot, player.id, dispatcher.user_data.get(player.id))        #TODO Это другой процесс
